    ...
    try {
      ...
      onToast(
        googleConnected
          ...
          : 'Evento creado (conecta Google Calendar para sincronizar)',
        'success',
      );
      setEventTitle('');
      setSelectedDate(null);
    } catch {
      onToast('Error al crear evento', 'error');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex justify-between items-center bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
        <div>
          ...
          {googleConnected && (
            <p className="text-xs text-emerald-600 font-semibold mt-1">Sincronizado con Google Calendar</p>
          )}
        </div>
      ...
      {selectedDate && (
        <div className="bg-gradient-to-r from-fuchsia-50 to-cyan-50 border-2 border-fuchsia-200 p-6 rounded-3xl animate-in slide-in-from-top-4">
      ...
      <div className="grid grid-cols-7 gap-2 md:gap-4">
        {WEEKDAYS.map((d) => (
          <div
            key={d}
            className="text-center text-[10px] font-black text-slate-300 uppercase tracking-widest mb-2"
          >
            {d}
          </div>
        ))}
        {grid.map((date, i) => {
          ...
          return (
            <div
              ...
              className={`min-h-[100px] md:min-h-[120px] bg-white border p-2 md:p-4 rounded-2xl transition-all cursor-pointer ${
                isSelected
            ...
            >
              <span
                className={`text-sm font-black inline-flex items-center justify-center w-8 h-8 rounded-lg ${
                  isToday
                  ...
                  <div key={event.id} className="group flex items-start gap-1">
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        onToast('Evento eliminado', 'success');
                      }}
